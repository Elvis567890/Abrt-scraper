...
The full scraper file you pasted is almost right, but the AI logs show that **`gemini-1.5-flash-latest` is not a valid model for your v1beta API key**, so those Gemini calls will always 404.[1][2]

I’ll give you a corrected full `scraper.py` with:

- `scrape_with_gemini` using a working v1beta model name (`gemini-1.5-flash-002`).[2][3]
- Multi-sport support kept (Football, Rugby, Basketball, etc.).  
- All your other scrapers unchanged.

You can paste this entire file over your current `scraper.py`.

***

## Corrected full `scraper.py` (with working Gemini model)
...
"""
        ...
        # Use a v1beta-compatible public Gemini model
        response = gemini_client.models.generate_content(
