...
Thanks for pasting your **current full scraper**. I’ll now add Football‑Data.org integration to this exact file, without changing your existing bookmaker scraping and arbitrage logic.

We’ll:

- Read `FOOTBALL_API_KEY` from the environment.
- Call `https://api.football-data.org/v4/matches` to get status + kickoff. [1][2]
- Filter out arbs where the match is `FINISHED`, `CANCELLED`, `POSTPONED`, etc., or too old.

Here is your **rewritten full code** with Football‑Data.org wired in.
...
FOOTBALL_API_BASE = "https://api.football-data.org/v4"  # [web:803][web:811]
MAX_MATCH_AGE_HOURS = 24  # how long after kickoff we still consider arb relevant
...
def get_match_status_and_kickoff_football_data(home_team, away_team, date_hint=None):
    """
    Use Football-Data.org to get official match status and kickoff (utcDate) for a given pair of teams.
    """
    ...
    headers = {
        "X-Auth-Token": FOOTBALL_API_KEY,  # auth header [web:810][web:813]
    }
    params = {
        "dateFrom": date_from,
        "dateTo": date_to,
        # You can add 'competitions' if you want to restrict leagues
    }
    ...
    target_home = normalize(home_team)
    target_away = normalize(away_team)

    for m in matches:
        home = m.get("homeTeam", {}) or {}
        away = m.get("awayTeam", {}) or {}
        home_name = _norm_team_obj(home)
        away_name = _norm_team_obj(away)

        if home_name == target_home and away_name == target_away:
            status = m.get("status", "").upper()  # FINISHED, SCHEDULED, etc. [web:807][web:811]
            kickoff_raw = m.get("utcDate")  # ISO timestamp [web:811][web:803]
...
def filter_opportunities_with_football_data(opps_list):
    ...
    Input: list of opportunities (your existing format).
    Output: filtered list with only opportunities that are not clearly finished / too old.
    """
    if not FOOTBALL_API_KEY:
        return opps_list
    ...
    for opp in opps_list:
        ...
        if not info:
            # If we can't find the match in Football-Data.org, keep it for now
            cleaned.append(opp)
            continue
...
# === bookmaker-specific scraping functions (unchanged) ===
...
def find_arbitrage(all_odds):
    ...
    # NEW: filter opportunities using Football-Data.org if key is set
