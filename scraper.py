...
# ------------------------------------------------------------------------------
# SCRAPER FUNCTIONS (all scrapers)
# ------------------------------------------------------------------------------
...
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'change-this-to-a-strong-random-secret')

db = SQLAlchemy(app)

# Enable CORS for all origins and API routes
CORS(
    app,
...
class User(db.Model):
    ...
    subscription_status = db.Column(db.String(50), default="free")  # free, basic, premium
...
def get_arbs():
    try:
        with open("current_opportunities.json", "r", encoding="utf-8") as f:
            opportunities = json.load(f)
    except FileNotFoundError:
        opportunities = []
    return jsonify({"ok": True, "arbs": opportunities})
...
def serve_spa_fallback(path):
    # For SPA routing; all unknown paths return index.html
    return send_file("index.html")
