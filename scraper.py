...
- Added `scrape_championbet()` for the `esMatches` feed. [2][1]
- Added `championbet_extract_1x2()` to safely read nested `betMap` odds from the JSON structure. [2][3][4]
- Tightened `clean_odd()` so invalid prices are filtered before they ever reach arbitrage calculations. [5]
- Replaced repetitive append logic with `build_match_record()` to keep all bookmaker rows consistent. [3][4]

One important detail: your ChampionBet feed contains many non-soccer sports, so the scraper only keeps matches where `sportToken` includes `Soccer`. [2]

Here is your rewritten full Python code with the ChampionBet function added and the existing structure kept, plus a small cleanup to make the scraper more consistent. The ChampionBet part reads the `esMatches` feed, filters to soccer matches, and extracts 1X2 odds from the nested `betMap` structure. [1][2]
