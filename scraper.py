...
def utc_timestamp() -> str:
    # ISO 8601 with Z, e.g. "2026-08-14T12:00:00Z"
...
def normalize_team(name: str) -> str:
    ...
    name = re.sub(r"\s+", " ", name)  # FIXED: backslash added
...
def match_key_similarity(key1: str, key2: str) -> bool:
    ...
    return teams_match(parts_one[0], parts_two[0]) and teams_match(
        parts_one[1], parts_two[1]
    )
...
class Payment(db.Model):
    ...
    manual_transaction_id = db.Column(db.String(255), nullable=False)
...
def signup():
    ...
    user = User(email=email, name=name or None, subscription_status="free", is_admin=False)
...
def submit_payment():
    ...
    if not transaction_id:
        return jsonify({"ok": False, "error": "Transaction ID is required"}), 400

    user = db.session.get(User, g.user_id)
    if not user:
        return jsonify({"ok": False, "error": "User not found"}), 404

    existing = Payment.query.filter_by(manual_transaction_id=transaction_id).first()
    if existing:
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Payment save error: %s", exc)
        return jsonify({"ok": False, "error": "Unable to save payment"}), 500

    return jsonify({
        "ok": True,
        "message": "Payment submitted for review. The administrator will activate your subscription.",
    }), 201
...
def admin_pending_transactions():
    ...
    for payment in pending:
        ...
        transactions.append({
            ...
            "manual_transaction_id": payment.manual_transaction_id,
            "created_at": payment.created_at.isoformat(),
        })
    return jsonify({"ok": True, "transactions": transactions})


@app.route("/api/admin/activate", methods=["POST"])
@admin_required
def admin_activate_transaction():
    data = request.get_json(silent=True) or {}
    ...
    if payment.status != "pending":
        return jsonify({"ok": False, "error": "Transaction is not pending"}), 409
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Activation error: %s", exc)
...
def admin_activate_by_email():
    ...
    email = str(data.get("email", "")).strip().lower()
    plan = str(data.get("plan", "")).strip().lower()
    if not email or plan not in PLAN_PRICES:
        return jsonify({"ok": False, "error": "Valid email and plan are required"}), 400
    ...
    user.subscription_expires_at = now + PLAN_DURATIONS[plan]

    try:
        db.session.commit()
    except Exception as exc:
        db.session.rollback()
        logger.exception("Manual activation error: %s", exc)
        return jsonify({"ok": False, "error": "Activation failed"}), 500

    return jsonify({"ok": True, "message": f"User {email} activated with {plan} plan."})
...
@token_required
def get_history():
    return jsonify({"ok": True, "history": load_arbitrage_history()})
...
def serve_admin():
    if not os.path.exists(ADMIN_FILE):
        return jsonify({"ok": False, "error": "admin.html was not found"}), 404
